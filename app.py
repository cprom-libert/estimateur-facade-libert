...
# --- CONFIGURATION ---
st.set_page_config(page_title="Estimateur Libert V21 (Expert)", layout="wide", page_icon="🏗️")

# ==============================================================================
# 🔑 CONFIGURATION API (A REMPLIR)
# ==============================================================================
try:
    GOOGLE_API_KEY = st.secrets["GOOGLE_API_KEY"]
except:
    GOOGLE_API_KEY = "" # Mettez votre clé ici si pas de secrets

# ==============================================================================
# 1. BASE DE PRIX "LIBERT 2025" (AVEC LOGIQUE HAUTEUR)
# ==============================================================================
DB_PRIX = {
    "LOGISTIQUE": {
        ...
        "MAJORATION_HAUTEUR": {"titre": "Majoration Grande Hauteur", "pourquoi": "Manutention supplémentaire au-delà de R+5.", "pu": 15.00, "unit": "m²"} # NOUVEAU
    },
    "PLATRE_ANCIEN": { 
    ...
    "ZINGUERIE": {
        ...
        "CHIEN_ASSIS": {"titre": "Habillage Chien-Assis", "pourquoi": "Rénovation zinc et jouées des lucarnes de toit.", "pu": 850.00, "unit": "U"} # NOUVEAU
    }
}
...
def get_facade_image(adresse, heading=0, pitch=10):
    """
    Récupère l'image avec un angle (Heading) ajustable pour être 'de face'.
    """
    if GOOGLE_API_KEY and len(GOOGLE_API_KEY) > 10:
        base = "https://maps.googleapis.com/maps/api/streetview"
        # fov=100 : Bon compromis zoom/largeur
        # heading : L'angle de la caméra (0=Nord, 90=Est...)
        return f"{base}?size=640x480&location={adresse}&fov=100&heading={heading}&pitch={pitch}&key={GOOGLE_API_KEY}"
    
    # Images de secours (inchangées)
...
# ==============================================================================
# 3. INTELLIGENCE ARTIFICIELLE (RECOUPEMENT DONNÉES)
# ==============================================================================
def proposition_ia_avancee(adresse):
    ads = adresse.lower()
    
    # CAS SPÉCIFIQUE DU 159 FAUBOURG ST ANTOINE (Correction Data)
    if "159" in ads and "antoine" in ads:
        return {
            "style": "Faubourien (Grand Gabarit)",
            "annee": "1860",
            "profil": "PLATRE_ANCIEN",
            "porte": "PORTE_COCHERE",
            "etages": 6,        # R+5 (Le RDC compte pour 1 niveau, donc 6 niveaux au total)
            "largeur": 16,
            "toiture": True     # Présence chiens-assis détectée
        }
    
    # AUTRES CAS (Logique Générale)
...
# --- SIDEBAR : RÉGLAGES TECHNIQUES ---
with st.sidebar:
    st.header("🔧 Calibrage Expert")
    
    # 1. REGLAGE CAMÉRA (NOUVEAU)
    st.subheader("📷 Angle de Vue (Façade)")
    cam_heading = st.slider("Orientation (Rotation)", 0, 360, 0, 10, help="Tournez la caméra pour être bien en face.")
    ...
    # Container qui sera rempli après
    container_params = st.container()

# --- PAGE PRINCIPALE ---
st.title("🔎 Estimateur Façade (Analyse Croisée)")
...
if st.session_state.data_ia:
    ...
    # MISE A JOUR SIDEBAR AVEC LES DONNÉES
    ...
    with c1:
        # Image dynamique avec les sliders
    ...
    if has_toiture:
        # Estimation : 1 chien assis tous les 6m de large
