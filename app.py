...
# ==============================================================================
# 2. BASE DE DONNÉES PRIX (LIBERT 2025)
# ==============================================================================
DB_PRIX = {
    "LOGISTIQUE": {
...
# ==============================================================================
# 3. MOTEUR TECHNIQUE (GPS, OSM, PHOTO)
# ==============================================================================
...
def query_osm_real_data(lat, lon):
    """Récupère les données du bâtiment sur OpenStreetMap"""
    query = f"""
    [out:json];
        ...
        if data['elements']:
            for el in data['elements']:
                if 'tags' in el:
                    ...
                    niveaux = t.get("building:levels", 0)
                    toit = t.get("roof:levels", 0)
                    
                    is_commerce = False
                    if t.get("shop") or t.get("building:use") == "retail":
                        is_commerce = True
...
def get_adresses_api(query):
    """Autocomplétion d'adresse (Corrigée)"""
...
def get_street_view(lat, lon, heading, pitch):
    """Génère l'URL de l'image Google"""
    if GOOGLE_API_KEY and len(GOOGLE_API_KEY) > 10:
        # FOV 80 pour éviter trop de distorsion
...
# --- CSS POUR RAPPORT HTML (STYLE BET PROPRE) ---
st.markdown("""
<style>
...
if st.session_state.step == 0:
    ...
    # Zone de saisie
    q_input = c1.text_input("Adresse du projet :", placeholder="Ex: 159 rue du faubourg saint antoine...")
    
    # Menu déroulant dynamique
    final_addr = None
    if q_input and len(q_input) > 4:
        opts = get_adresses_api(q_input)
        if opts:
    ...
    if c2.button("GÉNÉRER RAPPORT", type="primary"):
        if final_addr:
            with st.spinner("Analyse Bâtimentaire en cours..."):
...
if st.session_state.step == 1:
    ...
    with params_container:
        ...
        # Déduction du support par défaut
        def_idx = 1 
