...
# ==============================================================================
# 🔑 RÉCUPÉRATION SÉCURISÉE DE LA CLÉ GOOGLE
# ==============================================================================
try:
    # La clé doit être dans les Secrets de Streamlit Cloud
    GOOGLE_API_KEY = st.secrets["GOOGLE_API_KEY"]
except:
    # Fallback pour éviter le crash si pas de clé
    GOOGLE_API_KEY = ""

# ==========================================
# 1. BASE DE PRIX CORRIGÉE (C'est ici que l'erreur se trouvait)
# ==========================================
DB_PRIX = {
    "LOGISTIQUE": {  # J'ai renommé INSTALLATION en LOGISTIQUE pour correspondre aux appels
...
# ==========================================
# 3. INTELLIGENCE ARTIFICIELLE (SIMULATION)
# ==========================================
...
with col_search:
    search_query = st.text_input("Rechercher l'adresse :", placeholder="Ex: 159 rue du faubourg saint antoine...", value=st.session_state.adresse_input)
    if search_query and len(search_query) > 3:
        options = get_adresses_api(search_query)
        selected_address = st.selectbox("📍 Sélectionnez l'adresse exacte :", options) if options else search_query
    else:
...
if st.session_state.data_ia:
    ...
    with col_img:
        # IMAGE DE FACADE
        final_img = get_facade_image(selected_address, data['style'])
        st.image(final_img, caption="Façade identifiée", use_container_width=True)
    ...
    with col_txt:
        ...
        c3.metric("Ouvertures", f"{nb_fenetres} fenêtres")
    ...
    # Fonction pour afficher une ligne (Correction de l'erreur KeyError ici)
    def ligne_devis(icon, item_key, db_cat, qte, unit_ov=None):
        # On vérifie que la clé existe pour éviter le crash
        if db_cat in DB_PRIX and item_key in DB_PRIX[db_cat]:
            ...
            st.error(f"Erreur de configuration : Clé {item_key} introuvable dans {db_cat}")
            return 0

    # CALCULS (Utilisation des bonnes clés "LOGISTIQUE")
